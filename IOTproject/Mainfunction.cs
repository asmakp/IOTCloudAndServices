...
namespace IOTproject
{
    public static class Mainfunction
    {
        ...
        public static int Prevprecipitation;
        ...
        public static async Task Run([IoTHubTrigger("messages/events", Connection = "IotHubConnection", ConsumerGroup = "storage")] EventData message, ILogger log)
        {
            ...
            //  var pd = _endResult.ToString();

            string endResultjson = JsonConvert.SerializeObject(_endResult);
            log.LogInformation($"the data is  saved now to jsondoc format , Message:: {endResultjson}");
            HttpContent payload = new StringContent(endResultjson, Encoding.UTF8, "application/json");
            //  HttpContent payload = new StringContent(Encoding.UTF8.GetString(message.Body.Array), Encoding.UTF8, "application/json");
            // HttpContent payload = new StringContent(Encoding.UTF8.GetString(_endResult), Encoding.UTF8, "application/json");

            // log.LogInformation($"the data is  saved to jsondoc format , Message:: {payload }");
            ...
            if (precipitation > Prevprecipitation || precipitation < Prevprecipitation)
            {
